...
def main():
    ...
    api = API(ses)
    lp = BotsLongPoll(api, int(os.getenv('GROUP_ID')), version=0)
    ...
    async def edit_last_message(vk_bot: VkBot):
        await api.messages.edit(peer_id=int(vk_bot.peer_id), message=vk_bot.text, conversation_message_id=vk_bot.conversation_message_id)
